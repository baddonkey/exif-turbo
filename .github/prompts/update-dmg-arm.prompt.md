---
description: "Rebuild the macOS ARM DMG and upload it to an existing GitHub release"
name: "Update ARM DMG on existing release"
argument-hint: "version tag, e.g. 1.8.1"
agent: "agent"
---

Rebuild the macOS Apple Silicon (arm64) DMG for an existing release and upload it,
replacing any previous ARM macOS asset.
...
3. **Build the macOS ARM app bundle and DMG**
   - Run: `python scripts/build_macos.py --arch arm64`
   - This uses [exif-turbo-macos-arm.spec](../exif-turbo-macos-arm.spec) with `target_arch='arm64'`.
   - The expected output artifact is `dist/exif-turbo-<version>-macos-arm64.dmg`.
   - If the build fails, stop and show the error.

4. **Delete any existing ARM macOS DMG asset on the release**
   - List current assets: parse the `assets` array from the release metadata
     fetched in step 1.
   - For each asset whose name ends with `-macos-arm64.dmg`, delete it:
...
5. **Upload the new DMG**
   ...
   - Asset name: `exif-turbo-<version>-macos-arm64.dmg`
   ```
   curl -H "Authorization: token $TOKEN" \
        -H "Content-Type: application/octet-stream" \
        --data-binary @dist/exif-turbo-<version>-macos-arm64.dmg \
        "<upload_url>?name=exif-turbo-<version>-macos-arm64.dmg"
   ```
