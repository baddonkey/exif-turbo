...
| `models/accepted_tag_list_model.py`, `free_tag_list_model.py`, `marked_tag_list_model.py`, `pending_proposal_list_model.py`, `tgm_search_list_model.py` | Focused QML models for accepted controlled tags, current/remembered custom tags, all/some marked aggregation, ranked proposals, and canonical/alias TGM type-ahead results. |
...
| `qml/TaggingDrawer.qml` | Non-modal current-image drawer, available from Search/Browse via tag button or `Ctrl+T`: custom-tag creation/reuse/removal; canonical/alias TGM type-ahead; focused controlled-tag removal; proposal generation; scored accept/reject; progress and cancellation. |
