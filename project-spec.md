...
| **Portable tagging** | Store accepted canonical TGM terms in adjacent JSON sidecars and index them in FTS5 without changing originals |
...
| Module | Purpose |
|--------|---------|
| `image_index_repository.py` | `ImageIndexRepository` — all SQLCipher access for images, marks, sidecar synchronization state, normalized accepted tags/aliases, pending/rejected proposals, and FTS5. Tag mutations refresh only `tags_text`; image reindexing preserves it. Accepted labels, IDs, categories, vocabulary identity, and current aliases are searchable; proposals are excluded. Foreign-key cascades remove derived rows when an image row is removed but never delete a filesystem sidecar. |
...
| `tagging_service.py` | Single mutation boundary for manual acceptance, proposal decisions, and marked-image add/remove/auto-accept. Writes the sidecar first, updates derived cache transactionally, preserves valid sidecars after cache failures, and returns partial/conflict status. |
...
| `derivative_export_service.py`, `exif_metadata_writer.py` | Validate an output root outside indexed sources, preserve formats/relative trees, skip untagged/existing destinations, write accepted labels to XMP Subject and IPTC Keywords on temporary copies only, verify readback, and atomically publish or clean up. |
...
| `models/accepted_tag_list_model.py`, `marked_tag_list_model.py`, `pending_proposal_list_model.py`, `tgm_search_list_model.py` | Focused QML models for accepted tags, all/some marked aggregation, ranked proposals, and canonical/alias TGM type-ahead results. |
...
| `qml/TaggingDrawer.qml` | Non-modal right drawer, available from Search/Browse via tag button or `Ctrl+T`: canonical/alias type-ahead; focused accepted-tag removal; marked all/some aggregation and bulk add/remove; selected/marked proposal generation; scored accept/reject; confirmed optional auto-accept; marked derivative output picker; per-operation progress/cancel/error/summary. |
