...
- **Copy Tags** — copy the focused image's accepted controlled and custom tags to marked images, every current search result (including unloaded pages), or the current Browse folder. **Add** preserves existing target tags and merges missing source tags; **Replace** confirms before replacing target tags. The source image is always excluded.
