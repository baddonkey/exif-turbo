...
- **Tagged derivatives** — copy marked, tagged images to a user-selected folder outside indexed roots while preserving source formats and relative folder trees, then write controlled and custom labels to XMP Subject and IPTC Keywords on the copies only. Existing destinations and untagged images are skipped; originals and adjacent sidecars are not copied or modified.
