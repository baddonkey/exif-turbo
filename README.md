...
- **Non-destructive TGM tagging** — enable tagging per database, install the official Library of Congress TGM v1 XML distribution (with tagged-text fallback), and open the right-side workbench from Search or Browse with the tag button or **Ctrl+T**. Accepted canonical `loc-tgm:tgmNNNNNN` subject and genre/form terms are stored as plain JSON in adjacent `<image-filename>.sidecar.json` files; originals are never changed, and canonical labels, IDs, categories, and aliases participate in FTS5 search.
- **Review and bulk tagging** — search canonical TGM terms and aliases, add/remove accepted tags, generate and accept/reject CLIP proposals, and use persistent image marks for bulk add/remove, proposal generation, optional auto-accept, and derivative export. Proposal and auto-accept defaults are **0.24** and **0.32**; proposals require AI features, a separately built TGM term-vector index, and existing image vectors from **AI-Scan**.
- **Tagged derivatives** — copy marked, tagged images to a user-selected folder outside indexed roots while preserving source formats and relative folder trees, then write accepted labels to XMP Subject and IPTC Keywords on the copies only. Existing destinations and untagged images are skipped; originals and adjacent sidecars are not copied or modified.
